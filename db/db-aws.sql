...
--
-- Host: sang97.mysql.database.azure.com    Database: moviedb
-- ------------------------------------------------------
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `add_movie`(in title varchar(50), in director varchar(50), in rels int, in legth int, in price int, in imageUrl varchar(250), data json, in discription text)
begin 
    ...
    set @id_moive = 0;
    set @id_date = 0;
	insert into moviedb.movies(title, director, released, length, price, image, discription)
    ...
    while i < json_length(data) do
		...
		insert into moviedb.date(id_movie, date) values (@id_movie, json_extract(element_of_array, '$.date'));
        select max(id_date) from date into @id_date;
		set j = 0;
        while j < json_length(array_time)  do
			insert into moviedb.time_of_date( id_date, time) values (@id_date, json_extract(array_time, concat('$[',j,']')));
            set j = j + 1;
        end while;
	set i = i + 1;
    end while;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `adminEditMovie`(in idMoive int, in title char(20), in description text, in released int, in director char(30), in image_url char(255), in price int, in length int)
BEGIN
	UPDATE movies 
    SET title = title, 
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `admin_deleteMovie`(in id_m int)
BEGIN
	delete time_of_date
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `admin_getAllOrder`()
BEGIN
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `admin_getAllOrderByDate`(in d int)
BEGIN
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `admin_getAllOrderByMovie`(in id_m int)
BEGIN
	select reservation.id_order, user.name, movies.title, date.date, time_of_date.time, movies.price, seat.id_seat, reservation.time_order
    from ((((reservation inner join user on reservation.id_user = user.id_user)
					  inner join movies on reservation.id_movie = movies.id_movie)
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `admin_getAllOrderByTime`(in s_time int, in e_time int)
BEGIN
	select reservation.id_order, user.name, movies.title, date.date, time_of_date.time, movies.price, seat.id_seat, reservation.time_order
    from ((((reservation inner join user on reservation.id_user = user.id_user)
					  inner join movies on reservation.id_movie = movies.id_movie)
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `checkOrderUser`(in id_o int)
BEGIN
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `chonGioXem`(in id_m int, in _d int)
BEGIN
	select id_time, time
    from (movies inner join date on movies.id_movie = date.id_movie)
		  inner join time_of_date on date.id_date = time_of_date.id_date
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `choNgoiDaDuocDat`(in id_m int, in id_d int, in id_t int)
BEGIN
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `chonNgayXem`(in id_m int)
BEGIN
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `chonPhim`()
BEGIN
	select * from movies;
END ;;
DELIMITER ;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `datVe`(in id_u int, in id_m int, in id_d int, in id_t int, data json, in time_o varchar(30))
begin
declare i int default 0;
while i < json_length(data) do
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `doiChoNgoi`(in id_u int, in id_o int, in id_newSeat int)
BEGIN
	update reservation
...
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `getOneMovie`(in id_m int)
BEGIN
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `themNguoiDung`(in name varchar(50), in email varchar(50), in password varchar(255), in phone_number varchar(20))
BEGIN
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `userLogin`(in e varchar(50))
BEGIN
	select * from user
    where email = e; 
END ;;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `xemVeDaDat`(in id_u int)
BEGIN
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`admin`@`%` PROCEDURE `xoaVe`(in id_u int, in id_o int)
BEGIN
